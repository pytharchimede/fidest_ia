...
final class OcrDocumentService
{
    ...
    public function extract(array $file, ?int $apiClientId = null, ?string $clientReference = null, ?string $language = null): array
    {
        ...
        if (!$type) throw new RuntimeException('Le type documentaire GENERAL est indisponible.');

        $stored = $this->storage->store($file);
        $uuid = $this->uuidV4();
        $documentId = $this->documents->create([
            'uuid'=>$uuid,'document_type_id'=>(int)$type['id'],'api_client_id'=>$apiClientId,
            'client_reference'=>$clientReference,'original_name'=>$stored['original_name'],
            'stored_name'=>$stored['stored_name'],'storage_path'=>$stored['relative_path'],
            'mime_type'=>$stored['mime_type'],'file_size'=>$stored['file_size'],'sha256'=>$stored['sha256'],'status'=>'processing',
        ]);
        ...
        try{
            ...
            return [
                'document_id'=>$uuid,'internal_id'=>$documentId,'filename'=>$stored['original_name'],'mime_type'=>$stored['mime_type'],
                'ocr'=>['text'=>$text,'language'=>$language,'engine'=>$meta['engine']??'unknown','confidence'=>$confidence,'pages'=>max(1,$pages),'processing_time_ms'=>(int)round((microtime(true)-$started)*1000)]
            ];
        }catch(\Throwable $e){
