...
final class ServerResourceGuard
{
    ...
    {
        if (!$this->enabled) return $this->available([]);

        $metrics = $this->metrics();
        $reasons = [];

        $loadPerCpu = $metrics['load_per_cpu'] ?? null;
        if (is_float($loadPerCpu) && $loadPerCpu >= max(0.10, $this->maxLoadPerCpu)) $reasons[] = 'charge_cpu';

        $memoryPercent = $metrics['memory_percent'] ?? null;
        if (is_float($memoryPercent) && $memoryPercent >= max(1, min(100, $this->maxMemoryPercent))) $reasons[] = 'memoire';

        if ($reasons !== []) {
            return [
                'status'=>'paused','available'=>false,'busy'=>true,'paused'=>true,
    ...
    {
        $state=$this->status();
        if(!($state['available']??false)) throw new \RuntimeException('OCR_RESOURCE_BUSY: ressources serveur momentanément élevées. Réessayez dans quelques secondes.');
    }

    private function available(array $metrics): array
    {
        return ['status'=>'available','available'=>true,'busy'=>false,'paused'=>false,'message'=>'FIDEST IA est disponible.','retry_after'=>0,'metrics'=>$metrics];
    }
    ...
    private function memoryUsage(): ?array
    {
        ...
        foreach(preg_split('/\r?\n/',$content)?:[] as $line){
            if(preg_match('/^(MemTotal|MemAvailable):\s+(\d+)\s+kB$/',trim($line),$m)) $values[$m[1]]=(int)$m[2];
        }
        if(empty($values['MemTotal'])||!isset($values['MemAvailable'])) return null;

        $total=$values['MemTotal'];$available=max(0,$values['MemAvailable']);
