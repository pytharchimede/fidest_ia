...
final class ProcessRunner
{
    ...
    public function run(array $arguments,?array $environment=null):array
    {
        ...
        if($environment!==null){
            $current=getenv();
            $env=array_merge(is_array($current)?$current:[],$environment);
        }
        $pipes=[];$process=proc_open($command,[0=>['pipe','r'],1=>['pipe','w'],2=>['pipe','w']],$pipes,null,$env);
        if(!is_resource($process)) throw new RuntimeException('Impossible de démarrer le processus OCR.');
        fclose($pipes[0]);stream_set_blocking($pipes[1],false);stream_set_blocking($pipes[2],false);
        $stdout='';$stderr='';$started=microtime(true);$code=-1;
        try{
            while(true){
                $stdout.=stream_get_contents($pipes[1])?:'';$stderr.=stream_get_contents($pipes[2])?:'';
                $status=proc_get_status($process);
                if(!$status['running']){$code=(int)$status['exitcode'];break;}
                if(microtime(true)-$started>$this->timeoutSeconds){proc_terminate($process,9);throw new RuntimeException('Délai maximal du traitement externe dépassé.');}
                usleep(50000);
            }
        } finally {
