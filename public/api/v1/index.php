...
try{
    ...
    if($route==='/ocr/status'&&$method==='GET'){
        try{
            ...
            $state=method_exists($engine,'availabilityStatus')?$engine->availabilityStatus():['status'=>'available','available'=>true,'busy'=>false,'paused'=>false,'message'=>'FIDEST IA est disponible.','retry_after'=>0];
            respond(200,['success'=>true,'data'=>['ocr'=>$state],'meta'=>['request_id'=>$requestId]]);
        }catch(Throwable $e){
            respond(200,['success'=>true,'data'=>['ocr'=>['status'=>'unavailable','available'=>false,'busy'=>false,'paused'=>false,'message'=>'Le moteur OCR est indisponible.','retry_after'=>0]],'meta'=>['request_id'=>$requestId]]);
        }
    }
...
}catch(ApiException $e){$status=$e->status;$errorCode=$e->errorCode;respond($status,['success'=>false,'error'=>['code'=>$errorCode,'message'=>$e->getMessage()],'meta'=>['request_id'=>$requestId]]);}catch(RuntimeException $e){$resourceBusy=str_starts_with($e->getMessage(),'OCR_RESOURCE_BUSY:');$busy=$resourceBusy||str_starts_with($e->getMessage(),'OCR_BUSY:');$status=$busy?409:422;$errorCode=$resourceBusy?'OCR_RESOURCE_BUSY':($busy?'OCR_BUSY':'OCR_FAILED');$message=$resourceBusy?'FIDEST IA est temporairement en pause pour protéger les ressources du serveur. Merci de patienter.':($busy?'Je suis occupée en ce moment. Merci de patienter.':$e->getMessage());if($resourceBusy)header('Retry-After: '.max(1,(int)($config['resources']['retry_after_seconds']??15)));respond($status,['success'=>false,'error'=>['code'=>$errorCode,'message'=>$message],'meta'=>['request_id'=>$requestId,'retry_after'=>$resourceBusy?max(1,(int)($config['resources']['retry_after_seconds']??15)):null]]);}catch(Throwable $e){$status=500;$errorCode='INTERNAL_ERROR';$message=($config['app']['debug']??false)?$e->getMessage():'Une erreur interne est survenue.';respond(500,['success'=>false,'error'=>['code'=>$errorCode,'message'=>$message],'meta'=>['request_id'=>$requestId]]);}
